' 貼り付け先: F_SUB_全商品_チェック状況 のフォームモジュール【全文差し替え】
'   既存のコードを Ctrl+A で全選択して削除してから、このファイル全文を貼り付ける。
'   貼り付け後にデバッグ→コンパイルでエラーがないことを確認する。
'
' F_SUB_特定5講座_チェック状況 の 2026-07-31 版（動作確認済み）を
' 全商品向けに調整したもの（2026-08-04）:
'   ・一覧サブフォームの参照 Parent![F_SUB_特定5講座] を
'     Parent![F_SUB_全商品] に全置換（レター作成2ボタン・Form_BeforeInsert）。
'     コピー元のままだと F_全商品 上に F_SUB_特定5講座 という名前の
'     コントロールが無いため、エラー2465「フィールドが見つかりません」になる
'   ・処理内容そのものは特定5講座版から変更していない
...
Private Sub btn再注レター作成_Click()
    ...
    If Parent![F_SUB_全商品].Form![ショップ区分] = "通教" Then
        事業部 = "U-CAN"
        事業部TEL = "03-5388-6111"
    Else
        事業部 = "ライフ＆カルチャー"
    ...
    If IsNull(Parent![F_SUB_全商品].Form![年齢]) Then
        未成年FLG = False
    ElseIf Parent![F_SUB_全商品].Form![年齢] < 18 Then
        未成年FLG = True
    Else
        未成年FLG = False
    End If
    ...
    msg = msg & "No：" & Parent![F_SUB_全商品].Form![No] & vbCrLf
    msg = msg & "郵便番号：" & Parent![F_SUB_全商品].Form![郵便番号] & vbCrLf
    msg = msg & "住所：" & Parent![F_SUB_全商品].Form![都道府県] & Parent![F_SUB_全商品].Form![市区郡] & Parent![F_SUB_全商品].Form![町村番地] & Parent![F_SUB_全商品].Form![建物名] & vbCrLf
    msg = msg & "顧客名：" & Parent![F_SUB_全商品].Form![顧客姓_漢字] & " " & Parent![F_SUB_全商品].Form![顧客名_漢字] & vbCrLf
    msg = msg & "顧客番号：" & Parent![F_SUB_全商品].Form![顧客番号] & vbCrLf
    msg = msg & "事業部：" & 事業部 & vbCrLf
    msg = msg & "事業部TEL：" & 事業部TEL & vbCrLf
    msg = msg & "商品名：" & Parent![F_SUB_全商品].Form![商品名称] & vbCrLf
    ...
    If MsgBox(msg, vbYesNo + vbQuestion, "再注レター印刷情報の登録") = vbYes Then
        ...
        Parent![F_SUB_全商品].Form![再注レター] = True
        '明細側のフラグも確定して T_WORCS に保存する
        If Parent![F_SUB_全商品].Form.Dirty Then Parent![F_SUB_全商品].Form.Dirty = False
        CurrentDb.Execute sql, dbFailOnError
        If 未成年FLG Then
            MsgBox "印刷情報を未成年（" & Parent![F_SUB_全商品].Form![年齢] & "歳）として登録しました"
        Else
            MsgBox "印刷情報を登録しました"
        End If
    Else
...
Private Sub btn代引きレター作成_Click()
    ...
    If MsgBox(msg, vbYesNo + vbQuestion, "代引きレター印刷情報の登録") = vbYes Then
        ...
        Parent![F_SUB_全商品].Form![代引きレター] = True
        '明細側のフラグも確定して T_WORCS に保存する
        If Parent![F_SUB_全商品].Form.Dirty Then Parent![F_SUB_全商品].Form.Dirty = False
        CurrentDb.Execute sql, dbFailOnError
        If 未成年FLG Then
...
Private Sub Form_BeforeInsert(Cancel As Integer)
    Me![受注番号] = Parent![F_SUB_全商品].Form![受注番号]
    Me![商品コード] = Parent![F_SUB_全商品].Form![商品コード]
